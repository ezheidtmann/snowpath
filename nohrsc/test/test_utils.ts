...
export function getImageDataFromFixtureByUrl(url: string): Promise<ImageData> {
  ...
  let meta = JSON.parse(
    fs
      .readFileSync(path.join(__dirname, "..", "fixtures", `${filename}.meta`))
      .toString()
  );
  let dataBuffer = fs.readFileSync(
    path.join(__dirname, "..", "fixtures", `${filename}.imgData`)
  );
...
export function writeImageDataToFixturesByUrl(
  url: string,
  imgData: ImageData
): void {
  let filename = path.basename(url);
  ...
  fs.writeFileSync(
    path.join(__dirname, "..", "fixtures", `${filename}.meta`),
    JSON.stringify({
      width: imgData.width,
