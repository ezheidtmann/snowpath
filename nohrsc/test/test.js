...
describe("nohrsc parsing", function () {
  ...
  describe("overlay intersection", function () {
    ...
    let model = makeModelFromXMLFile(fixturePath("nohrsc_nsm_20200305.kml"));
    let matcher = model.getGroundOverlayMatcher(
      model.getFolderByName("Snow Depth")
    );

    it("should get intersecting overlay", function () {
      // coincident coordinates
      let overlays = matcher.getOverlaysIntersectingBbox([
        -75.04999999999966, 49.69999999999997, -70.78333333333316,
        53.96666666666647,
      ]);
      assert.ok(overlays.length > 0);

      let overlays = matcher.getOverlaysIntersectingBbox([-71, 50, -70, 54]);
      // smaller box also ok
      assert.ok(overlays.length > 0);

      let overlays = matcher.getOverlaysIntersectingBbox([
        -126.38671875, 74.54332982677906, -126.2109375, 74.59010800882325,
      ]);
  ...
  describe("tiler", function () {
    it("should get a tile", function () {
      // let folder = model.getFolderByName('Snow Depth');
      let tiler = new PointsTiler(
        fixtureContents("nohrsc_nsm_20200305.kml"),
        getImageDataFromFixtureByUrl
      );
